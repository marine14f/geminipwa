...
const appLogic = {
    ...
    // システムプロンプト更新処理 (元のscriptタグの最後のものを統合)
    updateCurrentSystemPrompt() {
        try {
            // state.settingsから直接値を取得するように修正
            const settings = state.settings || {};
            let prompt = settings.systemPrompt || "You are a helpful AI assistant."; // デフォルトプロンプト

            // ツール設定に応じてプロンプトを動的に変更するロジックは
            // API送信直前の `handleSend` 内で処理する方が一貫性があるため、
            // ここではユーザーが設定したデフォルトプロンプトを適用するに留める。
            // 将来的にUI上でプロバイダーを切り替える場合は、ここでの分岐が有効になる。
            
            state.currentSystemPrompt = prompt;
            
            // 現在のチャット画面に表示されているプロンプトも更新

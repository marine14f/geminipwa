...
- 同期対象にはFunction Callingで保存したキャラクター記憶やフラグも含まれます。Ver1.0で追加された自動送信メモリ（後述）と組み合わせることで、端末を跨いでもストーリーの整合性を保てます。
...
6.  PWAの設定画面にある`接続テスト`ボタンを押して成功すれば接続できています。

#### 他のトンネリングサービス
- [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/) や [LocalTunnel](https://theboroer.github.io/localtunnel-www/) など、HTTPSエンドポイントを提供するサービスであれば同様に利用できます。
...
### 品質管理オプション
- 「生成画像クオリティチェッカー」を有効化すると、Gemini Visionモデルが生成結果を自動評価し、プロンプトから逸脱していると判断した場合に再生成を試みます。

...
async function extractImageBlobFromMessage(message) {
    ...
    if (message.imageIds && message.imageIds.length > 0) {
        ...
        if (window.appLogic && typeof window.appLogic.getImageBlobById === 'function') {
            const imageData = await window.appLogic.getImageBlobById(imageId);
 ...
 * @param {string} [args.model]            - 明示モデル指定（"imagen-4.0-generate-001" | "imagen-4.0-ultra-generate-001" | "imagen-4.0-fast-generate-001" | "gemini-2.5-flash-image-preview" | "gemini-3-pro-image-preview" | "gemini-3.1-flash-image-preview"）
...
async function generate_image(args = {}) {
    ...
    try {
        ...
        if (modelName === "gemini-2.5-flash-image-preview" || modelName === "gemini-3-pro-image-preview" || modelName === "gemini-3.1-flash-image-preview") {
 ...
 * @returns {Promise<object>} 処理結果
 */
async function edit_image({ prompt, source_images, model = "gemini-3.1-flash-image-preview" }, chat) {
...

            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "model": {
                            "type": "STRING",
                            "description": "使用する画像編集モデルを指定します。指定がない場合はデフォルトの 'gemini-3.1-flash-image-preview' が使用されます。",
                            "enum": [
                                "gemini-2.5-flash-image-preview",
                                "gemini-3-pro-image-preview",
                                "gemini-3.1-flash-image-preview"
